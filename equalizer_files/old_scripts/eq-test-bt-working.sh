...
stop_eq() {
    ...
    [[ -n "$EQ_PID" ]] && wait "$EQ_PID" 2>/dev/null || true
    EQ_PID=""
}
...
visual_cue() {
    ...
    case "$type" in
        ...
        headphones)
            ...
            inputmodule-control led-matrix --pattern zigzag >/dev/null 2>&1
            ;;
        *)
            # Unknown source → flash everything briefly
            inputmodule-control led-matrix --pattern gradient >/dev/null 2>&1
            ;;
    esac
